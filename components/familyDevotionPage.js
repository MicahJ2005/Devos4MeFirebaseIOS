...
const FamilyDevotionPage = (devoTypeselected) => {
    ...
    const q = query(collection(params.db, "familydevotions"), where("userid", "==", params.runningUser.id), where("devodate", "==", reformattedDate));
    ...
    if(querySnapshot.size > 0){
      ...
      querySnapshot.forEach((doc) => {
        ...
        try {
            let contentToSend = `"Provide just a Bible verse about ${params.devotionTopicText} that can be used for Family Devotions"`;
            ...
            if(jsonScripture.choices[0].message.content != null){
                ...
                if(jsonTitle.choices[0].message.content != null){
                    ...
                    let contentToSend = `"Write a brief family devotional for the ${params.runningUser.lastname} family, based on the title ${jsonTitle.choices[0].message.content} and scripture ${jsonScripture.choices[0].message.content}. Try not to use any form of the word 'embrace' and do not add '[Your Name]' at the end"`;
                    const response = await fetch("https://api.openai.com/v1/chat/completions", {
                        method: "POST",
                        headers: {
                            'Accept': 'application/json',
                    ...
                    const newDevo = await addDoc(collection(params.db, "familydevotions"), {
